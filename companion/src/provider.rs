...
pub const SUPPORTED_PROVIDERS: [ProviderKind; 2] = [ProviderKind::PiShock, ProviderKind::OpenShock];
        ...
        match (self, action, target) {
            ...
            (Self::PiShock(client), TestActionKind::Sound, Some(target)) => {
                ...
                match target.data.as_ref() {
                    ...
                    Some(TargetData::OpenShock(_)) => Err(ProviderError::TargetProviderMismatch),
            ...
            (Self::OpenShock(client), TestActionKind::Sound, Some(target)) => {
                ...
                match target.data.as_ref() {
                    ...
                    Some(TargetData::PiShock(_)) => Err(ProviderError::TargetProviderMismatch),
                    None => Err(ProviderError::NotConnected),
                }
            }
            #[cfg(test)]
            (Self::Test(provider), TestActionKind::Sound, Some(target)) => {
...
mod tests {
    ...
    fn descriptors_are_exhaustive_for_current_providers() {
        for provider in SUPPORTED_PROVIDERS {
            assert_eq!(provider.descriptor().action_kind, ActionKind::Shock);
        }
    }
    #[test]
